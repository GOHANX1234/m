...
class MovieRepository(
    ...
    suspend fun getServerWatchlist(): List<LocalWatchlistItem> = withContext(Dispatchers.IO) {
        ...
        response.watchlist.mapNotNull { entry ->
            ...
            LocalWatchlistItem(
                ...
                targetType  = content.type,           // "movie" | "anime" | null
     ...
     *
     * Cast members are deduplicated by (lowercased) name, then **shuffled** so
     * the numbered ranking changes on every home-screen load, giving the row a
     * fresh Netflix-style feel each session.  Rank numbers are assigned after
     * the shuffle, so #1 is always a different person.
     *
     * Returns at most [limit] items (defaults to 20).
     */
    suspend fun getActors(limit: Int = 20): List<ActorItem> =
        withContext(Dispatchers.IO) {
            val movies = runCatching {
                service.getMovies(sort = "views", limit = 50).movies
            }.getOrDefault(emptyList())

            val seen   = mutableSetOf<String>()
            val actors = mutableListOf<ActorItem>()
            for (movie in movies) {
                for (cast in movie.cast.orEmpty()) {
            ...
            actors.shuffled().take(limit).mapIndexed { i, a -> a.copy(rank = i + 1) }
        }

    /**
     * Returns movies that feature [actorName] in the cast.
     *
     * Fetches two large pages (latest + popular) and filters client-side by cast
     * membership.  Falls back to an empty list if the API does not include cast
     * in list responses.
     */
    suspend fun getMoviesForActor(actorName: String): List<Movie> =
        withContext(Dispatchers.IO) {
            val batch1 = runCatching {
                service.getMovies(sort = "latest", limit = 50).movies
            }.getOrDefault(emptyList())
    ...
    /**
     * Fetch movies filtered by a specific genre name (e.g. "Hollywood", "Action").
     * Sorted by newest-added so the row is always fresh.
     */
    suspend fun getMoviesByGenre(genre: String): List<Movie> = withContext(Dispatchers.IO) {
        service.getMovies(genre = genre, sort = "latest", limit = 30).movies.map { it.toMovie() }
    }

    /**
     * Return up to 15 movies that share the first genre of [genres], excluding
     * [currentMovieId] itself.  Returns an empty list on any error so the caller
     * never has to handle a failure from this best-effort fetch.
     */
    suspend fun getSimilarMovies(currentMovieId: String, genres: List<String>): List<Movie> =
        withContext(Dispatchers.IO) {
            val genre = genres.firstOrNull() ?: return@withContext emptyList()
            runCatching {
                service.getMovies(genre = genre, sort = "rating", limit = 20).movies
