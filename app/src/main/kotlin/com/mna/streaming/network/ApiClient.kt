...
class ApiClient(private val sessionManager: SessionManager) {
    ...
    val animeApiService: AnimeApiService by lazy {
        buildRetrofit().create(AnimeApiService::class.java)
    }

    /**
     * Dedicated Retrofit instance for the public OTA update-check endpoint.
     *
     * Uses the same certificate pinner as [okHttpClient] (same hostname) but
     * intentionally excludes the cookie jar — the update endpoint is public and
     * requires no session credentials.  A shorter read timeout is used since the
     * check is a fast MongoDB lookup; we don't want a slow network to delay startup.
     */
